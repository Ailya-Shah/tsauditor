...
def test_single_large_gap_finance(clean_financial_df):
    ...
    drop_window = (df.index > target_date) & (df.index <= target_date + pd.Timedelta(days=9))
...
def test_sensor_domain_median_threshold(sensor_df):
    ...
    '''
    mid_idx = len(df) // 2
    drop_mask = (df.index > df.index[mid_idx]) & (df.index <= df.index[mid_idx] + pd.Timedelta(hours=23))
